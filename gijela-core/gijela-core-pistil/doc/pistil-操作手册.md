...
如果你是第一次接触 `pistil` 管理后台，可以先看这一段：

- 这是什么：一套面向后台管理场景的系统管理工作台
- 适合谁看：业务管理员、系统管理员、测试同学、交接接手同学
- 读完能做什么：知道各模块入口、完成用户/角色/菜单/组织等常见操作、快速定位会话与审计相关问题

建议阅读顺序：

1. 先看“系统访问”和“登录与退出”。
2. 再看“页面导航总览”，建立功能地图。
3. 按需查看用户、角色、菜单、组织、审计、会话、个人中心。
4. 遇到问题时，优先看“日常巡检建议”和“常见问题”。
...
为便于阅读和复现，这里说明本文的使用边界：

- 截图环境：本地联调环境 `http://localhost:5173/`
- 页面数据：来自本地初始化数据与当前测试数据
- 文档定位：说明“页面怎么用”和“功能入口在哪里”，不是后端接口设计文档
- 推荐读者：管理后台使用者、联调开发、测试同学

使用本文时，建议注意：

1. 页面有无数据与本地初始化数据、权限、筛选条件直接相关。
2. 若“有入口无数据”，优先检查数据初始化、账号权限和筛选项，而不是先改前端。
3. 本文截图用于说明操作位置，具体展示内容会随本地数据变化。
